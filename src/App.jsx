...
function App() {
  ...
  const handleAddFighter = (fighter) => {
    ...
    const index = zombieFighters.findIndex(f => f.id === fighter.id)
    zombieFighters.splice(index, 1)  
   }
   const handleRemoveFigther = (fighter) => {
      ...
      <ul>
        {team.map((fighter, index) => (
          <li key={index}>
            ...
            <p>Strenth: {fighter.strength}</p>
            <p>Agility: {fighter.agility}</p>
            <button onClick={() => handleRemoveFigther(fighter)}>Remove</button>
          </li>
        ))}
      </ul>
    </div>
    <div>
      <h3>List of Figthers</h3>
      <ul>
